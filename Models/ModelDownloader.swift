...
public actor ModelDownloader {
    ...
    private nonisolated func downloadFile(
        ...
        try HTTPJSON.validate(status: http.statusCode, url: url, allowing: [200, 206])
        var received = offset
...
actor DownloadThrottle {
    ...
    private func refill() {
        ...
        available = min(Double(limit), available + Double(limit) * last.duration(to: now).seconds)
        last = now
    }
}
...
final class DownloadProgressReporter: Sendable {
    ...
    private func update(force: Bool = false, _ mutate: (inout State) -> Void) {
        let progress: DownloadProgress? = state.withLock { s in
            ...
            s.lastEmit = now
            let windowSeconds = s.samples.first.map { $0.at.duration(to: now).seconds } ?? 0
