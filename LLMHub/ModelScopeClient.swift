...
public struct ModelScopeClient: Sendable {
    ...
    public static func parseRepoID(_ input: String) -> String? {
        ...
        guard parts.count >= 3, parts[0] == "models" else { return nil }
    ...
    private static func data(for request: URLRequest) async throws -> Data {
        ...
        case 404: throw HubError.notFound(request.url?.path ?? "")
        default: throw HubError.httpStatus(response.statusCode, url: request.url?.absoluteString ?? "")
        }
    }
}
