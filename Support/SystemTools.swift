...
public struct JavaScriptToolProvider: ToolProvider {
    ...
    public func execute(_ call: ToolCall) async throws -> String {
        ...
        guard let code = args["code"] as? String, !code.isEmpty else { return "error: missing code" }
    ...
    static func evaluate(_ code: String, timeLimit: TimeInterval, maxOutput: Int) -> String {
        ...
        return out.count > maxOutput ? String(out.prefix(maxOutput)) + "\n…[truncated]" : out
    }
...
public struct MacInfoToolProvider: ToolProvider {
    ...
    public func execute(_ call: ToolCall) async throws -> String {
        let args = (try? JSONSerialization.jsonObject(with: Data(call.argumentsJSON.utf8))) as? [String: Any] ?? [:]
        let topic = args["topic"] as? String ?? "overview"
        let text: String
        switch topic {
        ...
        return WebToolProvider.wrap(text, source: "mac_info: \(topic)")
    }
    ...
    static func overview() -> String {
        ...
        return [
            ...
            "Chip: \(sysctl("machdep.cpu.brand_string") ?? "unknown"), \(info.processorCount) cores (\(info.activeProcessorCount) active)",
...
public struct NetworkToolProvider: ToolProvider {
    ...
    public func execute(_ call: ToolCall) async throws -> String {
        ...
        case "http":
            text = await Self.http(URL(string: (args["host"] as? String).flatMap { $0.contains("://") ? $0 : nil } ?? "https://\(host)"))
        default:
...
/// Runs a stock tool with fixed arguments (no shell) and a timeout; stdout and stderr together.
enum ToolProcess {
    static func run(_ path: String, _ arguments: [String], timeout: TimeInterval = 20) async throws -> String {
        let box = ProcessBox(binary: URL(fileURLWithPath: path), arguments: arguments, stdin: nil)
