...
public struct WeatherToolProvider: ToolProvider {
    ...
    public func execute(_ call: ToolCall) async throws -> String {
        ...
        guard let location = (args["location"] as? String)?.trimmingCharacters(in: .whitespacesAndNewlines), !location.isEmpty else {
            return "error: missing location; ask the user which city"
        }
        let days = min(max(args["days"] as? Int ?? 3, 1), 7)
        do {
            ...
            return WebToolProvider.wrap(try Self.format(place: place, forecast: forecast), source: "get_weather: \(location)")
        } catch {
